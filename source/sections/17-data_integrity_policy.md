...
Wally takes data integrity very seriously. As stewards and partners of Wally Customers, we strive to assure data is protected from unauthorized access and that it is available when needed. The following policies drive many of our procedures and technical settings in support of the Wally mission of data protection.

Production systems that create, receive, store, or transmit Wally data (hereafter "Production Systems") must follow the guidelines described in this section.
...
1. All access to Production Server Systems must be logged. This is done following the Wally Auditing Policy.

## 17.4 Prevention of Malware on Production Systems

1. All Production Server Systems must have an intrusion detection agent running, and set to scan system every 2 hours and at reboot to assure no malware is present. Detected malware is evaluated and removed.
2. Virus scanning software is run on all Production Server Systems for anti-virus protection.
...
3. All Production Systems are to only be used for Wally business needs.
...
1. System, network, and server security is managed and maintained by the Security Officer in conjunction with the core engineering team.
...
4. Ensure Wally Customer Production Data is segmented and only accessible to Customers authorized to access data.
5. All Production Data at rest is stored on encrypted volumes. Encryption at rest is ensured through the use of automated scripts referenced in the [Configuration Management Policy](#9.-configuration-management-policy).
...
5. In the case of Wally provided APIs, provide mechanisms to assure person sending or receiving data is authorized to send and save data.
